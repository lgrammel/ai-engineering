...
In agent systems, a sandbox is where “tools” run (shell commands, code interpreters, file edits) so the agent can operate on a workspace without giving it full host privileges.
...
Why it matters: lets you execute untrusted code safely, reduce the blast radius of prompt injection or supply-chain attacks, and make tool execution more reproducible for debugging and evals.

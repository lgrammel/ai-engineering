# AI Agent

Synonyms: LLM agent, tool-using agent (sometimes "autonomous agent").

An AI agent is a system that uses an [LLM](./large-language-model.md) as a decision-making policy to run **tools** in a loop: observe context/state, choose an action (tool call or message), execute it, incorporate the result, and repeat until a stop condition is met.

See also: [Prompt](./prompt.md), [AI Gateway](./ai-gateway.md), [Sandbox](./sandbox.md), [Sandbox Service](./sandbox-service.md), [Observability Tools](./observability-tools.md), [Evaluations (Evals)](./evaluations-evals.md).

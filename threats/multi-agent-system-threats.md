...
- **Rogue agents**: An agent that operates outside its intended boundaries after being compromised (for example, via [prompt injection](./prompt-injection.md) or [context poisoning](./context-poisoning.md)) or due to [misaligned behavior](./misaligned-behaviors.md). Rogue agents are dangerous because their actions may appear legitimate when viewed in isolation -- the malicious intent only becomes visible when the full workflow is considered.

...
Key factors that affect latency include model size and architecture, input length, hardware (GPU type, memory bandwidth), [model quantization](./model-quantization.md) (lower precision reduces compute per token), [prompt caching](./prompt-caching.md) (skipping redundant prefill computation), and provider-side batching and load. [Streaming](./streaming.md) does not reduce actual generation time but lowers perceived latency by delivering tokens as they are produced. Latency, cost, and output quality form the primary tradeoff space for model selection and [inference provider](./inference-provider.md) choice; [observability](./observability.md) systems track these metrics in production to detect regressions and inform optimization.
